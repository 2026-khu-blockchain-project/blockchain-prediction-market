...
describe("PolyPredict", () => {
  ...
  describe("createMarket()", () => {
    it("오너만 시장을 생성할 수 있다", async () => {
      await expect(
        polyPredict
          .connect(alice)
          .createMarket("테스트", "설명", "카테고리", "YES", "NO", deadline + 100),
    ...
    it("과거 마감일로 시장 생성 불가", async () => {
      ...
      await expect(
        polyPredict.createMarket("테스트", "설명", "카테고리", "YES", "NO", pastDeadline),
    ...
    it("시장이 올바르게 생성된다", async () => {
      ...
      expect(market.category).to.equal("스포츠");
      expect(market.outcomeYes).to.equal("우승한다");
  ...
  describe("burnShares()", () => {
    ...
    it("YES 1개 + NO 1개 소각 시 1 USDC 반환", async () => {
      const before = await usdc.balanceOf(alice.address);
      await polyPredict.connect(alice).burnShares(marketId, USDC(1));
      const after = await usdc.balanceOf(alice.address);

      expect(after - before).to.equal(USDC(1));
      const [yes, no] = await polyPredict.getShareBalances(marketId, alice.address);
      expect(yes).to.equal(USDC(99));
      expect(no).to.equal(USDC(99));
    });

    it("잔고 부족 시 소각 실패", async () => {
      await expect(polyPredict.connect(alice).burnShares(marketId, USDC(101))).to.be.revertedWith(
  ...
  describe("resolveMarket()", () => {
    ...
    it("마감 전에 resolve 불가", async () => {
      const futureDeadline = (await networkHelpers.time.latest()) + 3600;
      await polyPredict.createMarket("새 시장", "설명", "카테고리", "YES", "NO", futureDeadline);
  ...
  describe("claimWinnings()", () => {
    ...
    it("중복 수령 불가", async () => {
      await polyPredict.connect(alice).claimWinnings(marketId);
      await expect(polyPredict.connect(alice).claimWinnings(marketId)).to.be.revertedWith(
        "No shares to claim",
      );
    });
    ...
    it("미정산 시장에서 claim 불가", async () => {
      ...
      await polyPredict.createMarket("새 시장", "설명", "카테고리", "YES", "NO", newDeadline);
      await expect(polyPredict.connect(alice).claimWinnings(1n)).to.be.revertedWith(
