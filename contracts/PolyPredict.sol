...
/**
 * @title PolyPredict
 * @dev Polymarket 스타일 CTF 예측 시장 — USDC 담보, ERC-1155 YES/NO 토큰
 */
contract PolyPredict is ERC1155, Ownable, ReentrancyGuard {
    ...
    event SharesMinted(uint256 indexed marketId, address indexed buyer, uint256 amount);
    ...
    event SharesBurned(uint256 indexed marketId, address indexed burner, uint256 amount);
    ...
    function burnShares(uint256 _marketId, uint256 _amount) external nonReentrant {
        Market storage m = markets[_marketId];
        ...
        require(balanceOf(msg.sender, yesId) >= _amount, "Insufficient YES balance");
        require(balanceOf(msg.sender, noId) >= _amount, "Insufficient NO balance");

        _burn(msg.sender, yesId, _amount);
        _burn(msg.sender, noId, _amount);
        ...
        emit SharesBurned(_marketId, msg.sender, _amount);
    }
    ...
    function getTokenIds(
        uint256 _marketId
    ) external pure returns (uint256 yesId, uint256 noId) {
        yesId = _marketId * 2;
        noId = _marketId * 2 + 1;
    }
}
