...
const MAX_USERS = 10;
const MAX_STAKE = parseUnits('1000', USDC_DECIMALS);

type Slot = { yes: bigint; no: bigint; virtualUsdc: bigint };

function emptySlot(): Slot {
  return { yes: 0n, no: 0n, virtualUsdc: parseUnits('50000', USDC_DECIMALS) };
}

function impliedYesBps(totalYes: bigint, totalNo: bigint) {
  const t = totalYes + totalNo;
  if (t === 0n) return 5000n;
  return (totalYes * 10000n) / t;
}

function multE4(totalYes: bigint, totalNo: bigint, sideYes: boolean) {
  if (sideYes) {
...
export function usePoolMarketDemo() {
  ...
  const placeBet = useCallback(
    async (yes: boolean, humanAmount: string) => {
      ...
      try {
        ...
        if (s.yes + s.no + amt > MAX_STAKE) throw new Error('유저당 최대 1,000 USDC');
        if (s.virtualUsdc < amt) throw new Error('가상 USDC 부족');
        if (yes && s.no > 0n) throw new Error('이미 NO에 걸었습니다.');
        if (!yes && s.yes > 0n) throw new Error('이미 YES에 걸었습니다.');
  ...
  const claimForActive = useCallback(async () => {
    ...
    try {
      ...
      let payout = 0n;
      if (outcome === 1) {
        if (snap.ty === 0n) throw new Error('YES 풀이 0');
        if (slot.yes === 0n) throw new Error('YES 스테이크 없음');
        payout = (pool * slot.yes) / snap.ty;
      } else {
        if (snap.tn === 0n) throw new Error('NO 풀이 0');
        if (slot.no === 0n) throw new Error('NO 스테이크 없음');
